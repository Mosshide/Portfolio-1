...
void Button::setBackgroundTxr(std::string txr)
{
	_bg.setTxr(txr);
}
...
{
	_text.setText(text);
	if (_shrinkwrap) setSize(_text.presence.w, _text.presence.h);
	realign();
}

void Button::setFont(int f)
{
	_text.setFont(f);
	_text.refreshText();
	if (_shrinkwrap) setSize(_text.presence.w, _text.presence.h);
...
void Button::setShrinkwrap(bool s)
{
	_shrinkwrap = s;
	_text.refreshText();
	if (_shrinkwrap) setSize(_text.presence.w, _text.presence.h);
...
void Button::realign()
{
	_text.setPositionCenter(presence.x + presence.w / 2, presence.y + presence.h / 2);
