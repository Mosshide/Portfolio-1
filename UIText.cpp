...
void UIText::setPositionCenter(GLfloat x, GLfloat y)
{
	presence.x = x - presence.w / 2;
	presence.y = y - presence.h / 2;
}
...
void UIText::refreshText()
{
	...
	toyBox->newTxtTxr(&_txr, text, rgba, _font);
	setSize(render->getTextWidth(text, _font), render->getFontSize(_font));
}
...
void UIText::setText(std::string t)
{
	if (text != t)
	{
		...
		glDeleteTextures(1, &_txr);
		SDL_Color rgba = { 255, 255, 255, 255 };
		toyBox->newTxtTxr(&_txr, t, rgba, _font);
		setSize(render->getTextWidth(t, _font), render->getFontSize(_font));
	}
}

void UIText::setFont(int f)
{
	_font = f;
	setText(text);
}